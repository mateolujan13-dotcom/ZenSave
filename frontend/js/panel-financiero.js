...
async function loadSummary() {
    ...
    const res = await window.API.get('/transacciones/resumen');
...
function renderProgress(expense, goal) {
    ...
    if (!goal || goal <= 0) {
        ...
        textEl.innerHTML = '<span class="col-span-2">No hay meta definida. <a href="ajustes.html" class="text-primary hover:underline">Ir a Ajustes</a></span>';
        return;
    }
    ...
    if (microData.length === 0) {
        listEl.innerHTML = `
            <li class="flex flex-col items-center justify-center py-8 text-on-surface-variant">
                ...
                <p class="text-sm">No hay gastos hormiga este mes</p>
            </li>
        `;
        return;
    }
