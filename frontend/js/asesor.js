...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayName = user.nombre ? user.nombre.split(' ')[0] : 'usuario';
...
async function sendMessage(text) {
    ...
    if (res.success) {
        ...
        conversationHistory.push({
            role: 'ai',
            text: `Hubo un problema de conexión con el sistema neuronal. (${res.error})`,
            time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        });
    }
...
async function loadChallenges() {
    ...
    const res = await window.API.get('/asesor/retos');
    if (res.success) {
        ...
        listEl.innerHTML = '';
        res.data.forEach(ch => {
            const progress = Math.min(100, (ch.actual / ch.objetivo) * 100);
                ...
                <div class="bg-surface rounded-xl p-3 border border-border-subtle card-glow">
                    <div class="flex justify-between items-center mb-1.5">
                        <div class="flex items-center gap-2 min-w-0">
                            ...
                            <h4 class="text-xs font-bold text-on-surface truncate" title="${ch.titulo}">${ch.titulo}</h4>
                        </div>
                    ...
                    <p class="font-data-display text-xs text-on-surface-variant mb-1.5">$${ch.actual.toLocaleString()} <span class="text-[10px] opacity-50">/ $${ch.objetivo.toLocaleString()}</span></p>
                    ...
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <button onclick='openDepositModal(${ch.id}, "${ch.titulo.replace(/'/g, "\\'")}")' class="text-primary hover:text-primary-fixed text-[11px] font-medium flex items-center gap-0.5 transition-colors">
...
async function handleCreateChallenge(e) {
    ...
    const res = await window.API.post('/asesor/retos', { titulo: title, objetivo: target });
    ...
    if (res.success) {
        ...
        window.showToast(res.error || 'Error al crear reto', 'error');
    }
}
...
async function confirmDeposit() {
    ...
    const res = await window.API.fetch('/asesor/retos/' + currentDepositChallengeId + '/depositar', {
        method: 'PATCH',
